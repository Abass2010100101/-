...
expiry_time = datetime.strptime('2090-1-12 00:00:00.000', '%Y-%m-%d %H:%M:%S.%f')
if current_time > expiry_time:
       print('انتهى التفعيل المجاني راسل المطور  @O770O7 • ')
       exit(0)
...
print(f"\033[1;31;40m{R}" * 70)
oi = render('venome',
                        colors=['red', 'green'], align='center')
print(oi)
print(f"\x1b[1;93m{K}" * 60)
print(" ~ By : @O770O7• ")
print(f"\033[1;36;40m•{a9}" * 60)
...
def em_num(user):
    ...
    try:
        ...
        priv = str(info.split('privateAccount":')[1]).split(',"')[0]
...
مـتـاح مـمـكن صـح و مـمـكن غلـط
        ...
        with open('nairobitik.txt', 'a') as f:
            f.write(ff + '\n')
...
مـتـاح مـمـكن صـح و مـمـكن غلـط
        '''
        with open('nairobihit.txt', 'a') as f:
            f.write(ff + '\n')    	
a = 0
...
def para():
	...
	cookies ={
        ...
        "sessionid":"d79c80b3c6aefb825cc7e9a218ababa2"
      }
...
def headd():
    ...
    cronet_date = f"{random.randint(2023, 2025)}-{random.randint(2, 12):02}-{random.randint(2, 28):02}"
    quic_version = f"{random.randint(10000000, 20000000)}"
    quic_date = f"{random.randint(2023, 2025)}-{random.randint(1, 12):02}-{random.randint(2, 28):02}"
